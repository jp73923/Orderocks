//
//  CheckProductExistScannerVC.swift
//  OrderRocks
//
//  Created by Jay on 19/02/22.
//
...
import Alamofire
import WebKit
class CheckProductExistScannerVC: UIViewController,AVCaptureMetadataOutputObjectsDelegate {
    ...
    override func viewDidLoad() {
        ...
        /*if let orderId = UserDefaults.standard.value(forKey: "OrderId") as? String {
            self.checkProductExistOrNot(strOrderId: orderId,strProductId: "1")
        }*/
    }
    func checkProductExistOrNot(strOrderId:String,strProductId:String) {
        let result = Constants.baseURL + "API/ProductExistInOrder?orderid=" + "\(strOrderId)" + "&productBarCode=" + "\(strProductId)"
        ...
        let task = URLSession.shared.dataTask(with: request) { data, response, error in
            ...
            if bool{
                DispatchQueue.main.sync {
                    let attributedString = NSAttributedString(string: "   Product Found", attributes: [
                    ...
                    imageView.image = UIImage.init(named: "ic_product_found")
                ...
                DispatchQueue.main.sync {
                    let attributedString = NSAttributedString(string: "   Product not found", attributes: [
                        NSAttributedString.Key.font : UIFont.systemFont(ofSize: 18), //your font here
                        NSAttributedString.Key.foregroundColor : UIColor.red
                    ])
                    
                    let imageView = UIImageView(frame: CGRect(x: 15, y: 5, width: 50, height: 50))
                    imageView.image = UIImage.init(named: "ic_product_not_found")
                    
                    let alert = UIAlertController(title: nil, message: nil, preferredStyle: UIAlertController.Style.alert) //Red text
                    alert.view.addSubview(imageView)
                    alert.setValue(attributedString, forKey: "attributedTitle")
    ...
    func metadataOutput(_ output: AVCaptureMetadataOutput, didOutput metadataObjects: [AVMetadataObject], from connection: AVCaptureConnection) {
        ...
        if self.allowedTypes.contains(metadataObj.type) {
            ...
            if metadataObj.stringValue != nil {
                ...
                if let orderId = UserDefaults.standard.value(forKey: "OrderId") as? String {
                    self.checkProductExistOrNot(strOrderId: orderId,strProductId: strProductId)
                }
